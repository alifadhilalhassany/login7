            ...
            <?php //هذا الكود راح يجيب البيانات من الداتابيس ويعرضها بلجدول ويحتوي على زرين 
                ...
                while ($row = mysqli_fetch_array($q)) {
                    echo 
                        ...
                        '<td>'.$row['password'].'</td>',
                        '<td>'.$row['per'].'</td>',
                        //url زر للحذف يدز الايدي بداخل ال 
                        '<td><a href="?rid='.$row['id'].'"> Delete </a></td>',
                        // url وزر للتعديل يحول المستخدم الى صفحة التعديل ويدز البيانات بداخل عنوان ال
                        '<td><a href="edit.php?update_id=' . $row['id'].'&update_name=' . $row['name'].'&update_username=' . $row['username'].'&update_per=' . $row['per'].'" > update </a></td></tr>';
                    }
                //url هذا الكود في حال اكو ايدي بعنوان ال 
                //راح يحذف البيانات من الداتابيس حسب هذا الايدي 
                if (isset($_GET['rid'])) {
                    $cmdd = 'DELETE FROM `aa` WHERE id ='.$_GET['rid'];
